#!/usr/bin/env python3
"""
客製化 3D 模型生成 - 真實牙齒版本（優化版）

優化項目：
  - compress_arch_spacing: KDTree 查詢批次化，delta 計算向量化
  - apply_customized_scaling: 牙齒中心計算向量化，減少逐顆迴圈開銷
  - remove_teeth_from_mesh: 改用 np.isin 向量化替代逐顆 face 遮罩
  - normalize_anterior_incisal_height: 分層寬度計算向量化
  - apply_tilt_correction / apply_incisal_curvature / apply_asymmetric_width: 不動（邏輯複雜，風險高）
  - align_upper_lower: 不動（trimesh transform，本身已快）
"""
...
TILT_MIN_CONFIDENCE    = 0.15
TILT_MAX_DEG           = 20.0
TILT_CORRECTION_BLEND  = 0.6
...
def align_upper_lower(upper_mesh, lower_mesh, flip_upper=True, rotate_180=False,
    ...
    if flip_upper:
        aligned_upper.apply_transform(
            trimesh.transformations.rotation_matrix(np.pi, [1, 0, 0], upper_center))
        upper_bounds = aligned_upper.bounds
        upper_center = upper_bounds.mean(axis=0)
        upper_bottom = upper_bounds[0, 2]
    ...
    for angle, axis in [(UPPER_TILT_CORRECTION_Y, [0, 1, 0]),
                        (UPPER_TILT_CORRECTION_Z, [0, 0, 1])]:
        if angle != 0:
            upper_center = aligned_upper.bounds.mean(axis=0)
            aligned_upper.apply_transform(
                trimesh.transformations.rotation_matrix(
                    np.radians(angle), axis, upper_center))
            upper_bounds = aligned_upper.bounds
            upper_center = upper_bounds.mean(axis=0)

    upper_center_xy = aligned_upper.bounds.mean(axis=0)[:2]
    lower_center_xy = aligned_lower.bounds.mean(axis=0)[:2]
    aligned_upper.vertices[:, :2] += lower_center_xy - upper_center_xy

    z_offset = lower_top + spacing - aligned_upper.bounds[0, 2]
    aligned_upper.vertices[:, 2] += z_offset

    if opening_angle != 0:
        upper_bounds = aligned_upper.bounds
        y_min, y_max = upper_bounds[0, 1], upper_bounds[1, 1]
        x_min, x_max = upper_bounds[0, 0], upper_bounds[1, 0]
        pivot_point  = np.array([
            x_min + (x_max - x_min) * ROTATION_PIVOT_X_RATIO,
            y_min + (y_max - y_min) * pivot_ratio,
            upper_bounds.mean(axis=0)[2]
        ])
        aligned_upper.apply_transform(
            trimesh.transformations.rotation_matrix(
                np.radians(opening_angle), [1, 0, 0], pivot_point))
...
def compute_customized_3d_scales(mesh, seg_labels, analysis_data, teeth_list):
    ...
    for tooth_id in teeth_list:
        ...
        if tooth_indices is None: continue
        ...
        tooth_id_str = str(tooth_id)
        tooth_dims   = analysis_data['teeth'][tooth_id_str].get('dimensions_3d', {})

        def calc_scale(measured, model_dim, axis_key):
            if measured is None or model_dim <= 0:
                return 1.0
            count = tooth_dims.get(axis_key, {}).get('count', 1)
            trust = 1.0 if count >= 2 else 0.3
            ratio = measured / model_dim
            all_scale_factors[axis_key].append(ratio)
            s = ratio ** (RATIO_ADJUSTMENT_STRENGTH * 0.5 * trust)
            return float(np.clip(s, MIN_RATIO_SCALE, MAX_RATIO_SCALE))

        scale_x = calc_scale(measured_width,  dims['width'],  'width')  if ENABLE_RATIO_ADJUSTMENT else 1.0
        scale_y = calc_scale(measured_height, dims['height'], 'height') if ENABLE_RATIO_ADJUSTMENT else 1.0
        scale_z = calc_scale(measured_depth,  dims['depth'],  'depth')  if ENABLE_RATIO_ADJUSTMENT else 1.0

        has_adjustment = any(abs(s - 1.0) > 0.02 for s in [scale_x, scale_y, scale_z])
        if has_adjustment:
            adjusted_count += 1
        ...
        tooth_scales[tooth_id] = {
            ...
            'model_dims': dims,
            'measured_width': measured_width, 'measured_height': measured_height,
            'measured_depth': measured_depth, 'has_adjustment': has_adjustment
        }

    print(f"  ✓ 需要調整的牙齒: {adjusted_count}/{len(tooth_scales)} 顆")

    overall_scale_x = overall_scale_y = overall_scale_z = 1.0
    if ENABLE_OVERALL_ADJUSTMENT:
        for axis_name, scale_list, var_name in [
            ('width', all_scale_factors['width'],   'x'),
            ('height', all_scale_factors['height'], 'y'),
            ('depth', all_scale_factors['depth'],   'z'),
        ]:
            if not scale_list: continue
            median_ratio = float(np.median(scale_list))
            adjusted     = float(np.clip(1.0 + (median_ratio - 1.0) * OVERALL_SCALE_STRENGTH,
                                         MIN_OVERALL_SCALE, MAX_OVERALL_SCALE))
            if var_name == 'x':   overall_scale_x = adjusted
            elif var_name == 'y': overall_scale_y = adjusted
            else:                 overall_scale_z = adjusted

        print(f"\n  ⭐ 三軸整體縮放: X={overall_scale_x:.3f}x  Y={overall_scale_y:.3f}x  Z={overall_scale_z:.3f}x")

    overall_scale = (overall_scale_x + overall_scale_y + overall_scale_z) / 3.0
    for tooth_id in tooth_scales:
        tooth_scales[tooth_id].update({
            'overall_scale':   overall_scale,
            'overall_scale_x': overall_scale_x,
            'overall_scale_y': overall_scale_y,
            'overall_scale_z': overall_scale_z,
        })
...
def apply_customized_scaling(mesh, seg_labels, tooth_scales):
    ...
    for tooth_id, scales in tooth_scales.items():
        if not scales['has_adjustment'] and scales.get('overall_scale', 1.0) == 1.0:
            continue
        tooth_indices = extract_tooth_vertices_indices(seg_labels, tooth_id)
        if tooth_indices is None: continue
        ...
        scale_vec   = np.array([scales['scale_x'], scales['scale_y'], scales['scale_z']])
        vertices[tooth_indices] = (tooth_verts - center) * scale_vec + center
        adjusted_count += 1
...
def compress_arch_spacing(mesh, seg_labels, compression=0.82):
    """★ 優化：牙齒 delta 批次計算，KDTree 查詢向量化"""
    compressed  = mesh.copy()
    vertices    = compressed.vertices.copy()
    arch_center = vertices[:, :2].mean(axis=0)
    ...
    # ★ 批次計算所有牙齒的 delta（一次 for 迴圈取代雙重迴圈）
    tooth_centers = {}
    for tooth_id in unique_teeth:
        indices = np.nonzero(seg_labels == tooth_id)[0]
        if len(indices) == 0: continue
        tooth_center = vertices[indices].mean(axis=0)
        dx = (tooth_center[0] - arch_center[0]) * (compression - 1.0)
        dy = (tooth_center[1] - arch_center[1]) * (compression - 1.0)
        delta[indices, 0] = dx
        delta[indices, 1] = dy
        tooth_centers[tooth_id] = (dx, dy)
    ...
    if len(gum_indices) > 0 and len(tooth_indices_all) > 0:
        ...
        gum_z_weight = np.clip((gum_verts[:, 2] - z_min) / z_range, 0, 1) ** 0.1
...
def apply_tilt_correction(mesh, seg_labels, analysis_data, teeth_list):
    ...
    for tooth_id in teeth_list:
        ...
        if tooth_id in {11, 21} or abs(correction_deg) < 0.5:
            skipped_count += 1; continue
        indices = np.nonzero(seg_labels == tooth_id)[0]
        if len(indices) == 0:
            skipped_count += 1; continue
    ...
    result_mesh = mesh.copy(); result_mesh.vertices = vertices
    return result_mesh
...
def apply_incisal_curvature(mesh, seg_labels, analysis_data, teeth_list):
    ...
    for tooth_id in teeth_list:
        if tooth_id not in INCISAL_TEETH: continue
        tooth_data = analysis_data['teeth'].get(str(tooth_id))
        if tooth_data is None: continue
        front_cp = None
        for m in tooth_data.get('measurements', []):
            if m.get('view') == 'front' and m.get('contour_cp'):
                front_cp = m['contour_cp']['points']; break
        if front_cp is None: continue

        p_msl = front_cp.get('mesial_third', {})
        p_mid = front_cp.get('incisal_mid', {})
        p_dst = front_cp.get('distal_third', {})
        x_msl = p_msl.get('x'); z_msl = p_msl.get('z_rel')
        x_mid = p_mid.get('x'); z_mid_cp = p_mid.get('z_rel')
        x_dst = p_dst.get('x'); z_dst = p_dst.get('z_rel')
        if any(v is None for v in [x_msl, z_msl, x_mid, z_mid_cp, x_dst, z_dst]): continue

        z_baseline = (z_msl + z_mid_cp + z_dst) / 3.0
        dz_msl = np.clip((z_msl - z_baseline) * BLEND_STRENGTH, -MAX_Z_SHIFT, MAX_Z_SHIFT)
        dz_mid = np.clip((z_mid_cp - z_baseline) * BLEND_STRENGTH, -MAX_Z_SHIFT, MAX_Z_SHIFT)
        dz_dst = np.clip((z_dst - z_baseline) * BLEND_STRENGTH, -MAX_Z_SHIFT, MAX_Z_SHIFT)
        if max(abs(dz_msl), abs(dz_mid), abs(dz_dst)) < 0.2: continue

        indices    = np.nonzero(seg_labels == tooth_id)[0]
        if len(indices) == 0: continue
        tooth_verts = vertices[indices]
        z_min, z_max = tooth_verts[:, 2].min(), tooth_verts[:, 2].max()
        z_range = z_max - z_min
        if z_range < 0.1: continue
        ...
        height_blend[crown_mask]  = ((tooth_verts[crown_mask, 2] - z_threshold) /
                                     (z_max - z_threshold + 1e-6))
        ...
        z_offsets[out_of_range] = np.clip(z_offsets[out_of_range], -MAX_Z_SHIFT * 0.5, MAX_Z_SHIFT * 0.5)
...
def apply_asymmetric_width(mesh, seg_labels, analysis_data, teeth_list):
    ...
    for tooth_id in teeth_list:
        if tooth_id not in ASYM_TEETH: continue
        tooth_data = analysis_data['teeth'].get(str(tooth_id))
        if tooth_data is None: continue
        front_cp = None
        for m in tooth_data.get('measurements', []):
        ...
        if x_msl is None or x_dst is None or x_mid_cp is None: continue
        ...
        if mesial_ratio > 0.85 or distal_ratio > 0.85:
            print(f"    #{tooth_id}: 跳過（cp異常）"); continue

        asymmetry    = mesial_ratio - 0.5
        scale_mesial = float(np.clip(1.0 - asymmetry * BLEND_STRENGTH,
                                     1.0 - MAX_SCALE_DIFF, 1.0 + MAX_SCALE_DIFF))
        scale_distal = float(np.clip(1.0 + asymmetry * BLEND_STRENGTH,
                                     1.0 - MAX_SCALE_DIFF, 1.0 + MAX_SCALE_DIFF))
        ...
        corrected_count += 1
        print(f"    #{tooth_id}: 近中{mesial_ratio:.2f} 遠中{distal_ratio:.2f} → {scale_mesial:.3f}/{scale_distal:.3f}")
...
scaled_upper_base, scaled_lower_base = align_upper_lower(
    ...
    flip_upper=UPPER_FLIP, rotate_180=UPPER_ROTATE_180,
    spacing=VERTICAL_SPACING, opening_angle=UPPER_JAW_OPENING_ANGLE,
...
upper_scales = compute_customized_3d_scales(scaled_upper_base, upper_seg_labels, analysis, upper_teeth)
lower_scales = compute_customized_3d_scales(scaled_lower_base, lower_seg_labels, analysis, lower_teeth)

scaled_upper_custom = apply_customized_scaling(scaled_upper_base, upper_seg_labels, upper_scales)
scaled_upper_custom = normalize_anterior_incisal_height(scaled_upper_custom, upper_seg_labels, analysis, upper_teeth)
scaled_upper_custom = compress_arch_spacing(scaled_upper_custom, upper_seg_labels, ARCH_COMPRESSION_RATIO_UPPER)
scaled_upper_custom = apply_tilt_correction(scaled_upper_custom, upper_seg_labels, analysis, upper_teeth)
scaled_upper_custom = apply_incisal_curvature(scaled_upper_custom, upper_seg_labels, analysis, upper_teeth)
scaled_upper_custom = apply_asymmetric_width(scaled_upper_custom, upper_seg_labels, analysis, upper_teeth)

scaled_lower_custom = apply_customized_scaling(scaled_lower_base, lower_seg_labels, lower_scales)
scaled_lower_custom = compress_arch_spacing(scaled_lower_custom, lower_seg_labels, ARCH_COMPRESSION_RATIO_LOWER)
scaled_lower_custom = apply_tilt_correction(scaled_lower_custom, lower_seg_labels, analysis, lower_teeth)
scaled_lower_custom = apply_incisal_curvature(scaled_lower_custom, lower_seg_labels, analysis, lower_teeth)
scaled_lower_custom = normalize_anterior_incisal_height(scaled_lower_custom, lower_seg_labels, analysis, lower_teeth)
scaled_lower_custom = apply_asymmetric_width(scaled_lower_custom, lower_seg_labels, analysis, lower_teeth)
...
report = {
    ...
    'overall_scale': {'upper': float(overall_scale_upper), 'lower': float(overall_scale_lower)},
    'tooth_scales': {
        str(k): {
            'scale_x': float(v['scale_x']), 'scale_y': float(v['scale_y']),
            'scale_z': float(v['scale_z']), 'overall_scale': float(v.get('overall_scale', 1.0))
...
print("="*70)
print(f"\n📊 生成摘要:")
print(f"  檢測到: {len(detected_teeth)} 顆  從未出現: {len(never_detected)} 顆")
